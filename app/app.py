...
country_options = [{"label": "World", "value": "World"}] + [
    {"label": location, "value": location} for location in country_values
]
default_countries = [
    "Brazil",
    "United States",
...
def _normalize_selected_countries(selected_countries):
    ...
    normalized = [
        str(country) for country in selected_countries if pd.notna(country) and str(country) in selection_values
    ]
...
def update_dashboard_views(selected_countries, end_date_value):
    ...
    if selected_countries == ["World"]:
        ...
        world_row = (
            latest_data[latest_data["location"] == "World"]
            if "location" in latest_data.columns
            else latest_data.iloc[0:0]
        )
        if (
            not world_row.empty
            and "total_deaths_per_million" in world_row.columns
            and pd.notna(world_row.iloc[0].get("total_deaths_per_million"))
        ):
            ...
            # Fallback: if only per-million available, compute population-weighted average
                ...
                "total_deaths_per_million" in non_agg.columns and "population" in non_agg.columns and not non_agg.empty
            ):
    ...
    summary_text = html.Div(
        children=[
            ...
            html.Span(
                ", ".join(selected_countries[:4]),
                className="selection-summary-countries",
            ),
            html.Span(
                ("" if len(selected_countries) <= 4 else f" +{len(selected_countries) - 4}"),
                className="selection-summary-more",
            ),
        ]
    )

    # `selected_count_text` was unused; return value uses formatted string directly.
    ...
    return (
        map_figure(
            ...
            metric=metric_value,
            color_max=color_max,
            selected_country_names=map_selected_countries,
        ),
            ...
            children=[
                html.Div(
                    children=[
                        ...
                        html.Div(
                            "Summary of indicators for the selected countries.",
                        ...
                        html.Div(
                            children=[
                                ...
                                html.Div(
                                    children=[
                                        html.Div(
                                            "Average Complete Vaccination Rate",
                                            className="kpi-title",
                                        ),
                ...
                html.Div(
                    children=[
                        html.Div(
                            "How ageing influenced CoVID-19 mortality?",
                            className="panel-title",
                        ),
                        html.Div(
                            "Bars: % population 65+; Color: cumulative deaths per million",
                            className="panel-caption",
                        ),
                        dcc.Graph(
                ...
                html.Div(
                    children=[
                        html.Div(
                            "Economic Disruption during the Pandemic",
                            className="panel-title",
                        ),
                        dcc.Graph(
                ...
                html.Div(
                    children=[
                        html.Div(
                            "Did stronger economies experience lower mortality?",
                            className="panel-title",
                        ),
                        dcc.Graph(
                ...
                html.Div(
                    children=[
                        html.Div(
                            "Economic Performance under Pandemic Pressure",
                            className="panel-title",
                        ),
                        dcc.Graph(
                            id="gdp-trend-graph",
