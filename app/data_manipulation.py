...
def _apply_style_template(fig, title_text=None, subtitle_text=None, height=450):
    ...
    fig.update_xaxes(gridcolor="rgba(15,23,42,0.05)", zeroline=False, showline=False)

    fig.update_yaxes(gridcolor="rgba(15,23,42,0.05)", zeroline=False, showline=False)
...
def gdp_alluvial_figure(filepath: Path = GDP_FILEPATH, top_n: int = 12, allowed_country_names=None):
    ...
    if missing_years:
        raise ValueError("GDP_data.csv precisa conter as colunas de 2019, 2022 e 2024 para o diagrama alluvial.")
    ...
    country_order = (
        df_long[df_long["year"] == latest_year].sort_values("gdp_usd", ascending=False)["Country Name"].tolist()
    )
    ...
    palette = px.colors.sample_colorscale(
        "Viridis",
        [0.35 + 0.45 * i / max(len(country_order) - 1, 1) for i in range(len(country_order))],
    )
    ...
    values_by_year = df_long.pivot_table(
    ...
    fig.update_layout(
        ...
        annotations=[
            ...
            dict(
                x=year_positions[middle_year],
                y=year_label_y,
                xref="paper",
                yref="paper",
                text=str(middle_year),
                showarrow=False,
                font=dict(size=14, color="#0f172a"),
            ),
            dict(
                x=year_positions[latest_year],
                y=year_label_y,
                xref="paper",
                yref="paper",
                text=str(latest_year),
                showarrow=False,
                font=dict(size=14, color="#0f172a"),
            ),
        ],
    ...
    return _apply_style_template(fig, height=layout_height)
...
def _resolve_country_selection(df_health: pd.DataFrame, selected_country_names=None, top_n: int = 6) -> list[str]:
    ...
    if selected_country_names:
        selected = [
            country for country in selected_country_names if country == "World" or country in available_countries
        ]
...
def _collapse_to_world_when_all_countries_selected(
    ...
    if (
        available_countries
        and set(selected_country_names) >= set(available_countries)
        and "World" in df_health["location"].values
    ):
        return ["World"]
...
def covid_evolution_figure(df_health: pd.DataFrame, selected_country_names=None):
    ...
    metric_specs = [
        (
            "new_cases_smoothed_per_million",
            "Incidence of Cases",
            "Number of new confirmed cases per million inhabitants, smoothed over 7 days",
            COLOR_PALETTE["primary"],
        ),
        (
            "new_deaths_smoothed_per_million",
            "Attributable Mortality",
            "Number of deaths from COVID-19 per million inhabitants, smoothed over 7 days",
            COLOR_PALETTE["secondary"],
        ),
        (
            "people_fully_vaccinated_per_hundred",
            "Vaccination Coverage",
            "Percentage of the population that has completed the primary vaccination scheme",
    ...
    country_colors = ["#1e40af", "#dc2626", "#0d9488", "#ea580c", "#7c3aed", "#0891b2"]
    color_map = {
        country: country_colors[index % len(country_colors)] for index, country in enumerate(selected_country_names)
    }
    ...
    for country in selected_country_names:
        ...
        grouped = country_df.groupby("month", as_index=False)[[spec[0] for spec in metric_specs]].mean(
            numeric_only=True
        )
    ...
    return _apply_style_template(fig, height=720)
...
def gdp_trend_figure(selected_country_names=None, filepath: Path = GDP_FILEPATH):
    ...
    country_colors = ["#1e40af", "#dc2626", "#0d9488", "#ea580c", "#7c3aed", "#0891b2"]
    color_map = {
        country: country_colors[i % len(country_colors)] for i, country in enumerate(df_long["Country Name"].unique())
    }
...
def _compute_padding_for_marker_size(max_size_value, sizeref, x_data_range, y_data_range):
    ...
    """
    # Removed: unused helper for marker padding (was not referenced anywhere).
    # Kept as a stub in case future logic requires it.
    return 0, 0
...
def gdp_mortality_scatter_figure(df_health: pd.DataFrame, selected_country_names=None, filepath: Path = GDP_FILEPATH):
    ...
    if selected_country_names == ["World"]:
        df_merged = (
            ...
            .merge(
                df_gdp[df_gdp["Country Name"] == "World"],
                left_on="location",
            ...
            if pd.isna(df_merged.iloc[0].get("total_deaths_per_million")):
                ...
                if not non_agg.empty and "total_deaths_per_million" in non_agg.columns:
                    ...
                    if not non_agg_clean.empty:
                        weighted_avg = (
                            non_agg_clean["total_deaths_per_million"] * non_agg_clean["population"]
                        ).sum() / non_agg_clean["population"].sum()
    ...
    # Marker sizing will be computed later from `population` (pop_mill),
    # so omit the earlier capped-size precomputation which was unused.
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(
                    ...
                    color=(
                        continent_colors.get(continent, COLOR_PALETTE["primary"])
                        if continent
    ...
    try:
        ...
        if x_all.size >= 2 and np.isfinite(x_all).sum() >= 2 and np.isfinite(y_all).sum() >= 2:
            ...
            # Calculate R^2 (not used in the chart directly)
    ...
    # Layout settings are applied by _apply_style_template; no local layout dict needed here.
...
def age_mortality_figure(df_health: pd.DataFrame, selected_country_names=None):
    ...
    if selected_country_names == ["World"]:
        ...
        if not world_row.empty and pd.isna(world_row.iloc[0].get("total_deaths_per_million")):
            ...
            non_agg = df_latest[~df_latest["location"].isin(["World", "International", "European Union"])].copy()
            if not non_agg.empty and "total_deaths_per_million" in non_agg.columns:
                non_agg_clean = non_agg.dropna(
                    subset=(
                        ["total_deaths_per_million", "population"]
                        if "population" in non_agg.columns
                        else ["total_deaths_per_million"]
                    )
                )
                if not non_agg_clean.empty:
    ...
    df_latest = df_latest[df_latest["location"].isin(selected_country_names)].dropna(
        subset=["median_age", "total_deaths_per_million"]
    )

    gdp_series = (
        pd.to_numeric(df_latest["gdp_per_capita"], errors="coerce")
    ...
    continents = [
        c for c in (df_latest["continent"].unique() if "continent" in df_latest.columns else [None]) if pd.notna(c)
    ]
    ...
    for continent in continents:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(
                    size=(
                        size_values_age.loc[df_continent.index].values
                        if "gdp_per_capita" in df_continent.columns
                        else 10
                    ),
                    color=(
                        continent_colors.get(continent, COLOR_PALETTE["primary"])
    ...
    df_latest["total_deaths_per_million"] = pd.to_numeric(
        df_latest.get("total_deaths_per_million", pd.Series(dtype=float)),
        errors="coerce",
    )
    ...
    fig.update_layout(
        ...
        font=dict(family=FONT_FAMILY, size=11, color=COLOR_PALETTE["neutral_dark"]),
        xaxis=dict(
            title="Share of population aged 65+ (%)",
            showgrid=True,
            gridcolor="rgba(15,23,42,0.06)",
        ),
    ...
    fig = _apply_style_template(
        fig,
        title_text="Age structure vs COVID-19 mortality",
...
def date_slider_marks(df_health: pd.DataFrame, num_marks: int = 6) -> dict:
    ...
    return {int(mark.value // 10**6): mark.strftime("%b\n%Y") for mark in marks}
...
def prepare_map_data(df_health: pd.DataFrame, end_date=None, preprocessed: bool = False) -> pd.DataFrame:
    ...
    if not preprocessed:
        df_map = df_map[
            df_map["continent"].notna() & (~df_map["location"].isin(["World", "International", "European Union"]))
        ].copy()
    ...
    df_map = df_map.groupby("location", as_index=False).tail(1)
...
def map_figure(
    ...
    if end_date is not None:
        ...
        if metric == "cfr_pct":
            ...
            title = f"World Map - Deaths until {end_date.strftime('%d/%m/%Y')}"
    else:
        if metric == "cfr_pct":
            title = "World Map - CFR (%)"
        elif metric == "total_cases":
    ...
    selected_country_names = [
        country for country in (selected_country_names or []) if country in set(df_plot["location"].dropna())
    ]
...
# Removed unused function: vaccination_comparison_figure
# This function was unused in the codebase and was removed to reduce dead code.
